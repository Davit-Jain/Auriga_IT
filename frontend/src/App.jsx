...
function App() {
  ...
  const handleFinePayment = async (event) => {
    event.preventDefault()
    const requestId = new FormData(event.currentTarget).get('requestId')
    const response = await fetch(`/api/reservations/${encodeURIComponent(requestId)}/fine-payment-request`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: '{}' }).catch(() => null)
    if (!response?.ok) {
      const error = response ? await response.json() : { error: 'The API is unavailable.' }
      setNotice(error.error)
      return
    ...
    await refreshData()
    setShowFine(false)
    setNotice(`Fine payment request ${requestId} sent for £${result.amountDue}.`)
    window.setTimeout(() => setNotice(''), 3500)
  }
  ...
  const approveRequest = async (requestId, returning = false, damagedQuantity = 0) => {
    const queuedRequest = adminRequests.find((request) => request.id === requestId)
    const finePayment = !returning && queuedRequest?.finePaymentStatus === 'Pending'
    const transfer = !returning && !finePayment && queuedRequest?.status === 'Transfer requested'
    const endpoint = returning ? 'return-approve' : finePayment ? 'fine-approve' : transfer ? 'transfer-approve' : 'approve'
    ...
    await refreshData()
    setNotice(returning ? `Item ${requestId} successfully submitted.` : finePayment ? `Fine for ${requestId} authorized.` : transfer ? `Transfer ${requestId} approved.` : `Request ${requestId} approved and issued.`)
    window.setTimeout(() => setNotice(''), 3500)
  }
    ...
    <div className="app-shell">
      <aside className="sidebar">
        ...
        <div className="sidebar-bottom">
          ...
          <div className="user-card"><div className="avatar">AV</div><div><strong>AV Coordinator</strong><span>Admin workspace</span></div><span className="more">...</span></div>
        </div>
      </aside>
      ...
      <main className="main-content">
        ...
        <div className="page-content">
          <section className="page-heading"><div><p className="eyebrow">WEDNESDAY, SEPTEMBER 16, 2026</p><h1>Good morning.</h1><p className="muted">Here is what is happening in your AV room today.</p></div><div className="heading-actions">{activeTab === 'Admin' && <button className="outline-button issue-button" onClick={() => setShowManageStock(true)}>Manage stock</button>}<button className="outline-button issue-button" onClick={() => setShowFine(true)}>Pay fine</button><button className="outline-button issue-button" onClick={() => setShowTransfer(true)}>Request transfer</button><button className="outline-button issue-button" onClick={() => setShowSubmit(true)}>Submit item</button><button className="primary-button" onClick={() => setShowBooking(true)}><span>+</span> New reservation</button></div></section>
      ...
      {showFine && <div className="modal-backdrop" onClick={() => setShowFine(false)}><div className="modal" onClick={(event) => event.stopPropagation()}><button className="modal-close" onClick={() => setShowFine(false)}>×</button><p className="eyebrow">FINE PAYMENT</p><h2>Submit fine payment</h2><p className="muted">Enter the request ID. The system calculates the outstanding late or damage amount and an admin authorizes the payment.</p><form onSubmit={handleFinePayment}><label>Request ID<input name="requestId" required placeholder="e.g. AV-1054" autoCapitalize="characters" /></label><div className="modal-actions"><button type="button" className="outline-button" onClick={() => setShowFine(false)}>Cancel</button><button type="submit" className="primary-button">Submit fine</button></div></form></div></div>}
